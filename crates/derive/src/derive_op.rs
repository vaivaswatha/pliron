...
use crate::{
    attr::{require_once, Attribute, DialectName, OpName},
...
/// Input for the `#[def_op]` proc macro.
struct DefOpInput {
    attrs: Attrs,
    ident: syn::Ident,
}
...
/// Attributes supported  by the `#[def_op]` proc macro.
///
/// The `dialect` attribute is extracted from the `op_name` attribute if the user used the dot
/// notation like `#[op_name = "dialect.op_name"]`.
struct Attrs {
    dialect: DialectName,
    op_name: OpName,
