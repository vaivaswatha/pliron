...
use crate::{
    attr::{require_once, Attribute, AttributeName, DialectName},
...
/// Input for the `#[def_attribute]` proc macro.
...
/// Attributes supported  by the `#[def_attribute]` proc macro.
///
/// The `dialect` attribute is extracted from the `attr_name` attribute if the user used the dot
/// notation like `#[attr_name = "dialect.attribute_name"]`.
struct Attrs {
    dialect: DialectName,
...
impl Attrs {
    fn from_syn(span: Span, input: &[syn::Attribute]) -> Result<Self> {
        ...
        for attr in input {
            if attr.path().is_ident(PROC_MACRO_NAME) {
                // remove the macro name as a potential attribute to other macros.
                continue;
